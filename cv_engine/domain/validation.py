    ...
    *,
    ...
    for index, claim in enumerate(claims):
        ...
        if claim.claim_id in seen_ids:
            groups["content"] = False
            issues.append(
                ValidationIssue(group="content", code="duplicate-claim-id", message=claim.claim_id)
            )
        seen_ids.add(claim.claim_id)
        if sha256_text(claim.text) != claim.text_hash:
            groups["content"] = False
            issues.append(
                ValidationIssue(group="content", code="claim-hash-mismatch", message=claim.claim_id)
            )
        if not claim.fact_ids and not (is_headline and claim.claim_type == "headline"):
            groups["content"] = False
            issues.append(
                ValidationIssue(group="content", code="unlinked-claim", message=claim.text)
            )
        ...
        for fact_id in claim.fact_ids:
            try:
                ...
                groups["content"] = False
                issues.append(
                    ValidationIssue(group="content", code="invalid-fact-link", message=str(exc))
                )
                continue
            if claim.claim_type == "canonical":
                ...
                if claim.text != expected_rendering:
                    ...
                    issues.append(
                        ValidationIssue(
                            group="content",
                            code="canonical-wording-mismatch",
                            message=(
    ...
    if draft.fact_store_version != facts.version:
        ...
        issues.append(
            ValidationIssue(
                group="content",
                code="fact-store-version-mismatch",
    ...
    for pattern, code in STALE_OR_UNSUPPORTED.items():
        if re.search(pattern, actual, re.IGNORECASE):
            ...
            issues.append(
                ValidationIssue(
                    group="content", code=code, message=f"Prohibited claim matches {pattern}"
                )
            )
    ...
    if draft.profile is not profile.profile or draft.track is not profile.track:
        ...
        issues.append(
            ValidationIssue(
                ...
                message="Draft and selected Profile disagree.",
            )
        )
    if draft.emphasis not in profile.allowed_emphases:
        ...
        issues.append(
            ValidationIssue(
                group="profile", code="emphasis-not-allowed", message=draft.emphasis.value
            )
        )
    if analysis.fit.value == "low" and analysis.user_override.get("fit") != "accepted-low-fit":
        ...
        issues.append(
            ValidationIssue(
                group="profile",
                code="low-fit",
                message="Low fit requires an explicit recorded override.",
            )
        )
    ...
    if historical_title_ids != heading_ids:
        ...
        issues.append(
            ValidationIssue(
                ...
                message="Historical titles must remain exact headings.",
            )
        )
