...
def _active_working_draft(connection: Connection, application_id: str) -> WorkingDraft:
    row = (
        connection.execute(
            select(working_drafts).where(
                working_drafts.c.application_id == application_id, working_drafts.c.active.is_(True)
            )
        )
        .mappings()
        .one_or_none()
...
def _approved_revision(connection: Connection, revision_id: str) -> ApprovedRevision:
    row = (
        connection.execute(select(approved_revisions).where(approved_revisions.c.id == revision_id))
        .mappings()
        .one_or_none()
    )
    if row is None:
...
def _approved_revisions(connection: Connection, application_id: str) -> list[ApprovedRevision]:
    rows = (
        connection.execute(
    ...
    *,
    ...
    row = (
        connection.execute(
            ...
            .select_from(
                working_drafts.join(
                    job_analyses, job_analyses.c.id == working_drafts.c.job_analysis_id
                )
                .join(selection_plans, selection_plans.c.id == working_drafts.c.selection_plan_id)
                .join(validation_runs, validation_runs.c.id == validation_run_id)
            )
            .where(working_drafts.c.id == working_draft_id)
            .with_for_update(of=working_drafts)
        )
        .mappings()
        .one_or_none()
    )
    ...
    if (
        ...
        raise PreconditionFailed(
            "approval validation does not match the exact working draft", code=VALIDATION_STALE
        )
    ...
    version = connection.execute(
        select(
            (func.coalesce(func.max(approved_revisions.c.version_number), 0) + 1).label("version")
        ).where(approved_revisions.c.application_id == application_id)
    ).scalar_one()
    connection.execute(
    ...
    changed = connection.execute(
        ...
        .where(working_drafts.c.id == working_draft_id, working_drafts.c.active.is_(True))
...
def _create_working_draft(
    ...
    *,
    ...
    _require_lineage(connection, application_id, job_analysis_id, selection_plan_id, source)
    connection.execute(
...
def _latest_approved_revision(connection: Connection, application_id: str) -> ApprovedRevision:
    row = (
        connection.execute(
            select(approved_revisions)
            .where(approved_revisions.c.application_id == application_id)
...
def _replace_active_working_draft(
    connection: Connection,
    application_id: str,
    job_analysis_id: str,
    selection_plan_id: str,
    ...
    *,
    ...
    if expected_working_draft_id is not None:
        if current is None or current["id"] != expected_working_draft_id:
            raise StateConflict(
                f"working draft {expected_working_draft_id} is no longer the active draft of application {application_id}"
            )
        if current["edit_version"] != expected_edit_version:
            raise StateConflict(
                f"working draft {expected_working_draft_id} is at edit version {current['edit_version']}, not {expected_edit_version}"
            )
    if current is None:
        draft_id = new_id()
        connection.execute(
...
def _working_draft(connection: Connection, working_draft_id: str) -> WorkingDraft:
    row = (
        connection.execute(select(working_drafts).where(working_drafts.c.id == working_draft_id))
        .mappings()
        .one_or_none()
...
def _lock_working_draft(connection: Connection, working_draft_id: str) -> WorkingDraft:
    row = (
        connection.execute(
            select(working_drafts).where(working_drafts.c.id == working_draft_id).with_for_update()
        )
        .mappings()
        .one_or_none()
    )
