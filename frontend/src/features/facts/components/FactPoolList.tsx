...
export const FactPoolList = ({
  ...
  >
    {groupBySource(entries).map(({ entries: grouped, source }) => (
      <section key={source}>
        <h3 className="sticky top-0 z-10 flex items-baseline justify-between gap-2 border-b border-cv-border bg-cv-surface-sunken px-3 py-1.5 text-support font-semibold text-cv-text-muted">
