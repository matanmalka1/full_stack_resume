...
## I. Approval-gated performance and architecture findings

These findings are executed serially. One item is completed and verified before the
next begins, and every next item requires explicit user approval. They do not silently
expand product semantics or weaken an existing acceptance gate.
