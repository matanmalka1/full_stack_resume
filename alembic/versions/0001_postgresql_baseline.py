...
Revision ID: 0001
Revises:
Create Date: 2026-09-24 17:37:24.680044
"""
...
DECLARE mutable_exceptions constant text[] := ARRAY[
  'applications', 'working_drafts', 'operations', 'operation_resource_leases',
  'operation_outputs', 'idempotency_receipts', 'knowledge_mutation_journal',
  'app_settings', 'payload_write_leases'
];
BEGIN
  ...
  END LOOP;
  FOREACH table_name IN ARRAY ARRAY['operations', 'operation_outputs', 'idempotency_receipts', 'knowledge_mutation_journal']
  LOOP
...
CREATE FUNCTION cv_guard_idempotency_receipt_completion() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NOT (OLD.status = 'pending' AND NEW.status = 'completed'
    AND OLD.id = NEW.id AND OLD.command_type = NEW.command_type
    AND OLD.idempotency_key = NEW.idempotency_key AND OLD.payload_json = NEW.payload_json
    AND OLD.payload_hash = NEW.payload_hash AND OLD.reserved_entity_id = NEW.reserved_entity_id
    AND OLD.created_at = NEW.created_at AND NEW.result_json IS NOT NULL
    AND NEW.completed_at IS NOT NULL) THEN
    RAISE EXCEPTION 'invalid idempotency receipt update';
  END IF;
  RETURN NEW;
END; $$;
CREATE TRIGGER valid_idempotency_receipt_completion BEFORE UPDATE ON idempotency_receipts
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("reserved_entity_id", sa.UUID(as_uuid=False), nullable=False),
        ...
        sa.UniqueConstraint(
            "command_type",
            "idempotency_key",
            name=op.f("uq_idempotency_receipts_command_type_idempotency_key"),
        ),
    )
    op.create_table(
    ...
    op.create_table(
        ...
        sa.Column("reclaim_deadline_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("committed_at", sa.DateTime(timezone=True), nullable=True),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.Column("id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("application_id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("job_analysis_id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("version_number", sa.Integer(), nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ...
            name=op.f("fk_selection_plans_application_id_job_analysis_id_job_analyses"),
        ),
        sa.ForeignKeyConstraint(
            ["application_id"],
            ["applications.id"],
        ...
        sa.UniqueConstraint(
            "application_id", "id", name=op.f("uq_selection_plans_application_id_id")
        ),
        sa.UniqueConstraint(
            "application_id",
            "version_number",
        ...
        sa.UniqueConstraint(
            "id", "job_analysis_id", name=op.f("uq_selection_plans_id_job_analysis_id")
        ),
    )
    op.create_table(
        "working_drafts",
        ...
        sa.Column("application_id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("job_analysis_id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("selection_plan_id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("parent_revision_id", sa.UUID(as_uuid=False), nullable=True),
        ...
        sa.ForeignKeyConstraint(
            ...
            name=op.f("fk_working_drafts_application_id_selection_plan_id_selection_plans"),
        ),
        sa.ForeignKeyConstraint(
            ["application_id"],
            ["applications.id"],
        ...
        sa.ForeignKeyConstraint(
            ...
            name=op.f("fk_working_drafts_selection_plan_id_job_analysis_id_selection_plans"),
        ),
    ...
    op.create_table(
        ...
        sa.Column("job_analysis_id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("selection_plan_id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("working_draft_id", sa.UUID(as_uuid=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("revision_id", sa.UUID(as_uuid=False), nullable=True),
        sa.CheckConstraint(
    ...
    op.create_table(
        "decision_records",
        sa.Column("id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("approved_revision_id", sa.UUID(as_uuid=False), nullable=False),
        sa.Column("structured_json", postgresql.JSONB(astext_type=sa.Text()), nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ...
            ["approved_revisions.id"],
            name=op.f("fk_decision_records_approved_revision_id_approved_revisions"),
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_decision_records")),
        sa.UniqueConstraint(
            "approved_revision_id", name=op.f("uq_decision_records_approved_revision_id")
        ),
    )
    op.create_table(
        "submissions",
        ...
        sa.Column("approved_revision_id", sa.UUID(as_uuid=False), nullable=True),
        sa.Column("artifact_version_id", sa.UUID(as_uuid=False), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("working_draft_id", sa.UUID(as_uuid=False), nullable=True),
        sa.Column("edit_version", sa.Integer(), nullable=True),
        sa.Column("content_hash", sa.Text(), nullable=True),
        sa.Column("job_snapshot_id", sa.UUID(as_uuid=False), nullable=True),
        sa.Column("job_analysis_id", sa.UUID(as_uuid=False), nullable=True),
        sa.Column("selection_plan_id", sa.UUID(as_uuid=False), nullable=True),
        ...
        sa.CheckConstraint(
            "edit_version IS NULL OR edit_version > 0",
            name=op.f("ck_validation_runs_edit_version_positive"),
        ),
        sa.ForeignKeyConstraint(
            ["application_id"],
            ["applications.id"],
    ...
    op.create_index(
        ...
        "validation_runs",
        ["artifact_version_id", "phase", sa.literal_column("created_at DESC")],
        unique=False,
    )
    op.create_index(
        "idx_validation_runs_draft",
        "validation_runs",
        ["working_draft_id", sa.literal_column("created_at DESC"), sa.literal_column("seq DESC")],
        unique=False,
    )
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index("idx_validation_runs_draft", table_name="validation_runs")
    op.drop_index("idx_validation_runs_artifact", table_name="validation_runs")
    ...
    for table_name in reversed(
        ("audit_records", "fact_events", "recruitment_events", "submissions", "validation_runs")
    ):
