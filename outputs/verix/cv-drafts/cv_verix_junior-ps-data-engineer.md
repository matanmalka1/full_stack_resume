---
created: 2026-06-01
modified: 2026-08-02
---

# Matan Malka

Backend-Oriented Full-Stack Developer · Python · SQL

Tel Aviv · +972-50-668-8386 · matan1391@gmail.com
[GitHub](https://github.com/matanmalka1) · [LinkedIn](https://www.linkedin.com/in/matanmalka1)

---

## Profile

Developer with hands-on production experience in Python, SQL, and PostgreSQL. Built and maintained data-heavy backend systems: relational schema design, complex query logic, automated data pipelines, and third-party API integrations. Strong analytical problem-solver with prior client-facing experience that bridges business requirements and technical delivery.

---

## Work Experience

### Full-Stack Developer — PH.Digital, Tel Aviv
**2025 – 2026**

- Designed and maintained relational database schemas in PostgreSQL; wrote complex SQL queries supporting business logic, reporting, and financial data processing.
- Integrated third-party REST APIs and external business systems, building automated workflows for document generation, client communication, and financial reporting processes: translating business requirements into reliable backend processes.
- Integrated third-party REST APIs and external systems to automate data ingestion and transformation workflows.
- Delivered features in Agile sprints, collaborating with product teams to translate requirements into production-ready solutions.

### Sales Team Leader — Pcom Solutions, Tel Aviv
**2019 – 2025**

- Led a team of 3–4 sales representatives; managed pipeline, KPIs, and enterprise client relationships.
- Grew team revenue 30% YoY.

---

## Education

### Full-Stack Development — John Bryce, Tel Aviv
**990 hours, 2024 – 2025**

- Covered Python (FastAPI, Django, Flask), relational and non-relational databases, and backend architecture.
- Built full-stack projects with AI/LLM integrations (OpenAI API, LLM pipelines); practiced Clean Code and SOLID principles.
- DevOps: Git, Docker, AWS EC2.

---

## Skills

| Category | Technologies |
|----------|-------------|
| Databases & SQL | PostgreSQL, MySQL, MongoDB, SQLAlchemy, SQL |
| Backend & Pipelines | Python (FastAPI, Flask), Node.js (Express), RESTful APIs |
| AI Integrations | OpenAI API, Anthropic API, Prompt Engineering, LLM integration |
| Frontend | React, TypeScript, JavaScript |
| Tools & Cloud | Git, GitHub, Docker, Postman, AWS EC2 |

---

## Languages
- Hebrew — Native · English — Fluent · Arabic — Conversational · French — Conversational
