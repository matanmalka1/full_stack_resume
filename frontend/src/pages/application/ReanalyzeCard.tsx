...
/* Re-running the analysis, beside the analysis it would replace - in the diagnostics tab,
   ...
   It used to sit in `ApplicationActions`, in the same row as "יצירת טיוטה": the one action
   ...
   it is the same `analyze` mutation `ApplicationActions` sends for a first analysis, only
   offered from where the analysis being reconsidered is on screen. `ApplicationActions`
...
export const ReanalyzeCard = ({
  ...
  const { analyze, plan, settings } = useApplicationActionsMutations(detail, onQueued);
