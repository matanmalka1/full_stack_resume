# מעקב ממצאי ניתוח וסיווג

רשימת עבודה על מנגנון חילוץ הדרישות, הסיווג וחישוב ה-Fit. **לא מפרט.** המפרטים
המחייבים ב־[`../spec/`](../spec/) גוברים, וההחלטות המאושרות נמצאות ב[רשומת
ההחלטות](../tailoring-decisions.md).

> **דרוש אימות מחדש מול D5.** הממצאים שלהלן אומתו מול הקוד לפני שהחלטה D5 העבירה
> את סמכות הכיסוי לספק והוציאה את `concept_for_quote` ממסלול ה-AI
> (`evidence.py`, `ai_extraction.py`). כל ממצא פתוח שנוגע לכיסוי, ל-`concept` או
> למילון ב-`config/requirements.json` — ובמיוחד `D7`, `A5`, `A6`, `A9` — צריך
> אימות חוזר מול הקוד הנוכחי לפני שמתחילים לתקן אותו. ממצא סגור לא נפתח מחדש
> בלי ראיה.
...
מותיר את שני התנאים `False`); **ציר 2** — `A8` (הסתירה מול `system-v4.md:33`,
...
```
פרופוזל שממלא **בדיוק** את הנחיית system-v4.md:33: `context_quote="Must have"`.
...
4. **RESOLVED — אין חוגה אחת. Stage 7 מתפצל לשלושה צירים, וכל אחד נשפט על הציר
   ...
   **ציר 2 — שער שסותר את הפרומפט של המערכת עצמה (`A8`, `A10`). מיישרים מול
   ...
   לא יותר:
   - `A8` — `system-v4.md:33` מורה לספק לצטט מרקר חובה ב-`context_quote`,
...
16. **RESOLVED — למחוק את ההבטחה ואת השדה.** ‏`topic_tags` ירד מ-
    ...
    **מה שהכריע, ונמצא בקוד ולא במסמך:** (א) ההצהרה השנייה
    ...
    (ג) הפרומפט (`system-v4.md`) לא מזכיר תגיות בכלל, ולכן הספק מעולם לא
