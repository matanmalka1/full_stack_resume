...
const FactListItem = ({ fact, recordedStatus }: FactListItemProps) => {
  ...
  return (
    <li className="p-4 hover:bg-cv-surface-muted">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <StatusBadge className="shrink-0 px-2.5 py-0.5" icon={factStatusIcons[fact.status]} tone={factStatusTones[fact.status]}>
      ...
      {auditMismatch ? (
        <Callout
          ...
          tone="blocker"
        >
          יש להפעיל בדיקת התאמה לפני שימוש נוסף.
        </Callout>
      ) : null}
    </li>
  );
          ...
          {items === undefined ? null : (
            ...
            >
              {items.map(({ fact, recorded_status: recordedStatus }) => (
                <FactListItem fact={fact} key={fact.fact_id} recordedStatus={recordedStatus} />
              ))}
            </ul>
          )}
        </QueryState>
