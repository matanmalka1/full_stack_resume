# CV Engine Provider Contract v8
...
- `propose_analysis`: read the posting once. Classify track, profile, emphasis, and
  ...
  requirements. Use `unknown` for uncertainty, not `unsupported`. For `matched`, use
  shortfall severity `none` and a null reason. For `unsupported`, use `material`. For `unknown`, use
  ...
  whether the employer's demand is met; otherwise use `unknown`. State the exact missing
  condition in `shortfall_reason`. Never infer numeric proximity unless both the required
  value and the candidate's held value are traceable to the supplied structured evidence.
