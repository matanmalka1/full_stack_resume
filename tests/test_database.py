...
def test_status_history_and_transition_contract(tmp_path: Path) -> None:
    repo = Repository(tmp_path / "applications.sqlite3")
...
def test_immutable_job_snapshot_trigger(tmp_path: Path) -> None:
    repo = Repository(tmp_path / "applications.sqlite3")
...
def test_next_action_is_not_a_status(tmp_path: Path) -> None:
    repo = Repository(tmp_path / "applications.sqlite3")
