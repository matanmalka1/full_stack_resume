...
lower what you propose, so proposing a coverage you cannot evidence gains nothing and
costs the whole output.
...
- `propose_requirement_extraction`: read what the posting actually requires. For each
  ...
  Then say whether the candidate meets it. `coverage` is your reading of the supplied
  canonical facts against this requirement: `matched` when they verify it, `partial`
  when they verify part of it, `unsupported` when they do not verify it, and
  `undetermined` when you cannot tell from the facts you were given. Cite the facts you
  read in `evidence`, each with the fact ID exactly as supplied and a short rationale
  naming what in that fact answers the requirement. A tag or a job title that merely
  sounds related is not evidence; a fact whose meaning does not answer the requirement
  is `unsupported`, not `matched` with a hopeful rationale. `matched` and `partial`
  require at least one cited fact. Never name a fact ID that was not supplied.

  For a `threshold` requirement, report the level the posting demands in `demanded` -
  a bare number of years ("5"), or the named level it asks for ("fluent") - and cite
  the fact carrying the candidate's own value. The comparison itself is recomputed from
  that fact; a `coverage` you report is not what decides it.

  For an `any-of`/`all-of` requirement, answer per member in `members_coverage`, keyed
